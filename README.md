...
默认分析维度
字段画像：字段类型、唯一值、缺失率、内存占用
数值洞察：均值/中位数/标准差/极值、波动特征
分类洞察：TopN 类别分布、占比、集中度
时间趋势：自动识别时间列并分析趋势、波动、同比环比
相关性分析：自动输出 Pearson 相关性矩阵、强相关指标提示
输出格式
...
🛠️ 故障排除
常见问题
Q: 中文显示乱码 A: 系统缺少中文字体，安装PingFang SC或Microsoft YaHei

Q: Excel文件读取失败 A: 检查文件格式和编码，确保文件未损坏

Q: 截图生成失败 A: 检查Chromium安装状态，运行npx playwright install chromium

Q: 内存不足 A: 减少单次处理数据量，分批处理大文件
